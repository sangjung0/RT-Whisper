...
from data import Sentence, Context, Token
...
class Composer(Pipeline):
    ...
    def can_process(self, context: Context) -> bool:
        if (
            len(context.merged_completed_tokens) == 0
            and len(context.merged_candidate_tokens) == 0
        ):
            return False
        return (
    ...
    def compute_process(self, param):
        (completed_tokens, language, order, completed_length) = param
