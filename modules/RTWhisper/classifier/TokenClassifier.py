...
class TokenClassifier(Classifier):
  ...
  def compute_process(self, param):
    ...
    prev_audio_start = max(0, merged_processed_audio_sc - self.__MAX_PREV_SC["default"])
    anchor = 0
    ...
    sc_offset += anchor
    merged_completed_tokens = prev_completed_tokens + [t for t in merged_candidate_tokens if t.end < sc_offset]
    prev_candidate_tokens = [
      t for t in merged_candidate_tokens if t not in merged_completed_tokens and t.is_word
    ]
  ...
  def apply_process(self, context:Context, result:tuple):
    ...
    context.prev_candidate_tokens = context.merged_candidate_tokens = prev_candidate_tokens
